...
#pragma multi_compile __ _STEREOMODE_TOPBOTTOM _STEREOMODE_LEFTRIGHT
...
#pragma multi_compile __ UNITY_SINGLE_PASS_STEREO STEREO_INSTANCING_ON STEREO_MULTIVIEW_ON
	...
	float4 GetStereoDebugTint(bool isLeftEye)
	{
		...
		//#if _STEREOMODE_TOPBOTTOM | _STEREOMODE_LEFTRIGHT
		if (_StereoMode == 1 || _StereoMode == 2)
		{
			float4 leftEyeColor = float4(1, 0, 0, 1);		// green
			float4 rightEyeColor = float4(0, 1, 0, 1);		// red
